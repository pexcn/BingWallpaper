...
- 换壁纸时**淡入淡出**而不是硬切：画在桌面图标下面，图标和窗口都不会被盖住，六种填充方式都支持
...
- 自备图的日期**优先从文件名里取**：`20250101` / `2025-01-01` / `2025_01_01` 都认，带分隔符时
  月和日可以只写一位（`2025-1-1`、`2025_1_12`）；出现在名字的哪个位置都行
  （`IMG_20250101_120000.jpg` 是最常见的形状），认不出来才退回文件修改时间。
  拷进来的文件，修改时间往往就是「拷贝那一刻」，一个目录拷进来会全挤成同一天，排序等于没排；
  名字里那串数字才是图片真正的日期。
- 认出日期之后，**格子里的标题会把日期那段从文件名里去掉**：`IMG_20250101_120000.jpg` 显示成
  `IMG_120000`，`20250101_sunset.jpg` 显示成 `sunset`。日期本来就单独占一行，标题里再写一遍是重复。
  剪完不剩什么的保留原名：什么都不剩（`20250101.jpg`），或者只剩语言代码、序号这类两字符以内的
  ASCII 片段（`20210603-zh.jpg` 的 `zh`、`20210603-zh-cn.jpg` 的 `zh-cn`）——它们比完整文件名
  说的还少。两个字的中文不受这条限制，`20250101_台北.jpg` 就显示 `台北`。除此之外不做美化：
  不改大小写、不把下划线换成空格——标题还得能和 Explorer 里的文件名对上。
- Bing 自己的图，`favorites.txt` 里没有标题时（比如整个收藏目录从别的机器拷过来）标题用文件名里的
  图片 id，`20250101_WhyteCliffP_UHD.jpg` 显示成 `WhyteCliffP`，日期和分辨率后缀都不再出现在标题里。
  「是我们的图」这里按 `_UHD` / `_1920x1080` 后缀判定，比右键菜单用的那套严：`20250101_a_beach.jpg`
  只是形状撞上了，按普通自备图处理，显示 `a_beach` 而不是把 `a` 当成图片 id。
- 托盘菜单最上面那行是当前壁纸的日期和标题，菜单有多宽就由它决定，所以**它不会太长**。
  Bing 的标题是一句话，砍一半也还读得懂，超长就直接截断；只能从文件名推出来的标题砍一半
  只剩噪音，索性整条丢掉，显示成 `[2025-01-01] 的壁纸`（方括号表示锁定中）。
  完整名字还在鼠标悬停的提示里，也在「选择壁纸」窗口里。
...
- 换壁纸的淡入淡出是自己画的：`SPI_SETDESKWALLPAPER` 只让资源管理器重画壁纸层，那一下就是硬切，
  ...
  再把盖子的 alpha 走到 0——新图就从旧图底下浮出来。
  顺序是先换后淡：淡出开始时壁纸已经是对的，之后任何一步出岔子最坏也只是变回硬切
- 盖子上的画面是**从壁纸层直接拷贝像素**（`GetDC(WorkerW)` + `BitBlt`），不是重新解码原图：
  一次显存内拷贝只要几毫秒，而读一张 3~8 MB 的 UHD JPEG、全尺寸解码、再重采样到全屏，
  合计上百毫秒且全压在 UI 线程上。拷来的画面与资源管理器画的**逐像素一致**，
  也不必复刻排版规则，于是**六种填充方式都能淡入淡出**。
  「按填充方式从文件渲染」那条老路已经删掉：排版规则是资源管理器的，复刻出来只可能是近似，
  而近似的盖子撤掉时会跳一下，比没有渐变更难看；何况有六种，写错了也不会有人发现。
  DWM 允许拒绝这种拷贝（返回全黑），所以拷完采样九个点，全同色就判定失败，直接硬切并记 `WARN`
- 盖子放在**壁纸层**而不是最顶层：向 `Progman` 发未文档化消息 `0x052C`，资源管理器会把桌面拆成
  「图标窗口 + 身后的 WorkerW」（它自己给幻灯片壁纸做淡入淡出用的就是这个拆分），
  把盖子作为那个 WorkerW 的分层子窗口挂进去。于是图标压在盖子上面、依旧可点，
  任何一个真实窗口都像遮住壁纸一样遮住它——不会出现「换个壁纸糊你一脸」。
  找不到 WorkerW 就退回 `Progman` 并压到 z 序最底；再找不到就硬切
- 每帧只改一个 alpha 字节，位图只上传一次，合成交给 DWM：整个淡出对 CPU 几乎没有开销。
  如果每帧用 GDI+ 做一次全屏 alpha 混合，4K 下单帧就是几十毫秒，根本淡不动
- `SystemParametersInfoW` 放在线程池上跑：它返回前 Windows 要把图**再完整解码一遍并重新编码**，
  写出 `TranscodedWallpaper`，满血机器上两三百毫秒，降频时轻松过一秒。它不碰本程序的任何窗口，
  没有留在 UI 线程上的理由；而这段时间盖子正不透明地把桌面挡得严严实实，
  于是「界面卡住」变成了「界面空闲」
- **盖子跑在自己的一条 UI 线程上**，建窗口、跑消息循环、跑淡出全在那条线程，淡完线程就退出。
  动画和主线程在忙什么本来就没有关系，绑在一起只是省事：淡出的每一帧靠 `WM_TIMER` 驱动，
  而 `WM_TIMER` 是消息队列里优先级最低的一种，队列里只要还有输入或绘制就轮不到它——
  连点时排队的鼠标消息足以把整个淡出饿死，而且**饿死了也不留痕迹**：
  alpha 是按时钟算的，一帧没画到的淡出照样按时结束。单独一条线程上没有别的消息，问题从根上消失。
  顺带把「跨进程挂子窗口导致两边输入队列绑定」也隔离到这条只活半秒的线程上，不再挂在主线程上
- 连点时**不新建盖子，交给正在跑的那个**：盖子上画的是「这一串点击开始前桌面的样子」，
  而它淡掉之后露出来的是「此刻资源管理器正在画的东西」。所以连点十下的结果是
  **一次从第一张平滑淡到最后一张的过渡**，中间那几张一次都不用画
- 如果点击落在**渐变已经开始之后**，先把此刻屏幕上的画面**折进盖子自己的位图**，再把 alpha 拉回 255：
  屏幕上正在合成的是「盖子的图 × alpha + 底下的壁纸 ×(1−alpha)」，
  用互补的 alpha 把壁纸 `AlphaBlend` 进盖子的位图，得到的正是这张合成图——
  于是恢复不透明时**一个像素都不动**，新壁纸照样换在看不见的地方，渐变从自己的中点接着往下走。
  这一次混合只在「中途接到改动」时发生，不是每帧；每帧仍然只改一个 alpha 字节
- 收藏页连点是**记住最后一张**，不是排队也不是丢弃：排队要坐等 N 次转码才停在最后一张，
  丢弃则会停在最先点的那张——恰恰是用户改了主意不要的那张。于是连点十下只跑两次「转码 + 写注册表」
- 淡出前按住不动的时长是量出来的，不是猜的：`hold = clamp(转码实测耗时, 150, 500) ms`。
  `SystemParametersInfoW` 返回只代表转码完成，资源管理器是在自己的线程上把新图画上去的，
  画完不通知任何人，也没有可靠信号可等。太早降 alpha，透出来的会是**旧**壁纸——正是要藏的那一下。
  转码慢就说明这台机器此刻慢、资源管理器画得也会慢，所以拿它当尺子：降频时自动放宽，满血时不多等
- 淡出本身 500 ms。系统给幻灯片壁纸做的大约 1 秒，这里明显更短——换壁纸是对一次点击的回应，点击要的是回应
- 建窗口时带 `WS_EX_NOPARENTNOTIFY`：`CreateWindowEx` 会**同步发送** `WM_PARENTNOTIFY` 给父窗口，
  而父窗口是资源管理器的，那一条消息就足以让建窗口卡在另一个进程上。这是这条路径上唯一的同步等待点
- 测量和建窗口时临时把线程切到 `PerMonitorV2`（`SetThreadDpiAwarenessContext`）：进程是按系统 DPI 感知的，
  在多显示器不同缩放时拿到的显示器矩形是主屏的坐标系，而壁纸层是物理像素，盖子会盖不满
- 代价是清楚的：全屏位图在 4K 下有几十 MB（所以淡完立刻释放，不留给下一次）。
  嫌它多事就在 INI 里 `FadeTransition=false`
