...
- 「最近」列表是 15 天而不是一次请求能拿到的 8 天：接口一次最多回 8 条（`n` 只认 1~8），
  但 `idx` 会把窗口整体往前推——响应总是从「今天 − idx」起往回数 n 天。`idx` 在 7 处封顶
  （实测 `idx=8`、`idx=10` 与 `idx=7` 返回完全一致），于是 `idx=0` 与 `idx=7` 两次请求就覆盖了
  今天到今天 − 14，正好重叠一天。两次请求**串行**发出：第一页带着今天那张，刷新真正要用的就是它，
  第二页失败只记 `WARN` 并沿用第一页的 8 天，桌面上什么都不会发生。合并按 `startdate` 去重，
  所以哪天 `idx` 不再生效、两页内容变得一样，也只是自动退回 8 天窗口，不会冒出重复磁贴
...
- 收藏夹以目录为唯一事实来源：「是不是收藏」就是「文件在不在 `favorites\`」，条目数、大小、排序
  ...
  `favorites.txt` 只缓存 Bing 图的标题和版权链接——接口只回溯 15 天，这两样过期就再也拿不回来，
...
- 退出这两种模式时桌面上怎么处理，两者相反：关掉轮播会立刻换上当天的图并重置检查间隔——轮播是
  按自己的节奏在换图，停下来总得有个落点；取消锁定则什么都不动，当前这张留在桌面上，等检查间隔
  的定时器下次到点再换成当天的。因为锁定本身也不换图，只是记下桌面上已经有的那张，解锁自然也不
  该换：否则同一个开关一个方向没有副作用、另一个方向把用户特意挑出来的图冲掉，还没法撤销
- 轮播开关放在选择窗口「收藏」页的标题栏，而不是设置窗口里：它和锁定一样说的是「程序此刻在做什么」，
  不是一项偏好，放在它轮播的那份列表上方才对得上；轮播间隔留在设置窗口——快慢确实是偏好。
  收藏夹为空且轮播未开时开关置灰：没有东西可轮播的「开启」什么都不会发生，从外面看与点击没生效无从分辨。
  开关画成胶囊而不是按钮：那一行里再没有别的胶囊，滑块停在哪边就是模式在哪边，不必点一下才知道当前是开是关
...
- 播放到哪儿不落盘：那是本次会话的状态，写出去要么把配置文件变成状态文件，要么给每次换壁纸
  添一次磁盘写。重启即新的一轮，从外面看与一次洗牌没有区别。新一轮的第一张在启动时立刻贴出，
  并且排在首次刷新之前：刷新一开始就占住「正在处理」的状态，而换图撞上它只会被丢掉、不会重排，
  排在后面的话桌面得等满一个间隔才动
...
- 最近 15 天的缩略图按绘制它的格子宽度解码，而不是必应给的 400x240：15 张常驻位图从约 5.6 MB
  降到约 1.4 MB（100% DPI），缩放也从每次重绘一次变成每张图一次
