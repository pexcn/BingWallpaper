...
- 换壁纸时**淡入淡出**而不是硬切：画在桌面图标下面，图标和窗口都不会被盖住（仅「填充」模式）
...
FadeTransition=true       ; 换壁纸时淡入淡出: true 或 false；只在 Fit=Fill 时有效
...
- 换壁纸的淡入淡出是自己画的：`SPI_SETDESKWALLPAPER` 只让资源管理器重画壁纸层，那一下就是硬切，
  系统没有任何过渡可以开。做法是先把「当前这张」按填充方式渲染到一张全屏位图，盖在桌面上，
  在盖住的状态下换壁纸，再把盖子的 alpha 走到 0——新图就从旧图底下浮出来。
...
- **只有「填充」有淡入淡出**，其余五种填充方式仍是硬切：盖子必须和 Windows 接下来要画的
  一模一样，撤掉时才不会跳一下，为此得复刻每一种排版规则；填充既是默认值，规则又只有一行
  （等比放大到铺满、居中、多出来的裁掉，且逐显示器各算各的），性价比只有它划算
- 淡出前先按住 120 ms 不动：`SystemParametersInfoW` 返回只代表转码完成，资源管理器是在自己的线程上
  把新图画上去的，而且画完不通知任何人。太早降 alpha，透出来的会是**旧**壁纸——正是要藏的那一下
...
- 代价是清楚的：跨进程挂子窗口会让两边的输入队列绑在一起（盖子只活半秒，所以可以接受），
  以及全屏位图加解码后的图片在 4K 下有几十 MB（所以淡完立刻释放，不留给下一次）。
