...
- 托盘菜单的「上一张 / 下一张」在最近 8 天里翻；锁定的是收藏夹里的图片时，改在收藏夹里翻
...
- 当前壁纸在 `favorites\` 里时（从收藏页点一张就是这种状态），托盘菜单的「上一张 / 下一张」
  改在收藏夹里前后翻，顺序与收藏页一致（新→旧），翻到的那张同样保持锁定。到头了点了就没反应——
  收藏夹的边界只有枚举目录才知道，不值得为了把菜单项画成灰色而每次都去读一遍目录。
