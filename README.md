...
- 最近 8 天的历史壁纸缩略图网格，点击任意一张即设为壁纸并锁定
...
- 刷新期间到来的触发不再被丢弃，而是记一笔、等当前这轮结束后补跑一次。滚轮扫过地区下拉框会
  逐格触发，以前是 `if (busy) return`，于是只有第一格生效、真正选中的那格被丢掉，
  INI 写着一个地区、桌面上却是路过的另一个。补跑只需一次：下一轮重新读配置，
  自然落在最终选中的地区上（所以是一个标志位，不是队列）
