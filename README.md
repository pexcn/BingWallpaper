...
- **取消收藏 = 把图片移回 `wallpapers\`**，并重新按 `KeepDays` 计时：一张收藏了半年的图移回去
  不会立刻被当成过期图删掉。界面上的日期仍是图片本身的日期，改的只是清理用的文件时间。
...
- 自备图的日期**优先从文件名里认**（`20250101` / `2025-01-01` / `IMG_20250101_120000.jpg` 都行），
  认不出来才退回文件修改时间——拷进来的一整个目录，修改时间往往全挤成同一天，排序等于没排。
  格子里的标题就是文件名去掉日期那段，`20250101_sunset.jpg` 显示成 `sunset`。
