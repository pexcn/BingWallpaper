...
- 托盘菜单深浅两套主题共用一个渲染器，两边只差颜色不差画法；浅色的颜色直接取自系统
...
- 托盘菜单深浅两套主题共用一个渲染器，而不是「浅色用框架的、深色用自绘的」——两个渲染器会漂移，
  而漂移出来的地方（分隔线多长、勾选态怎么标）恰好是截图一对比就露馅的。高亮条、描边、弹出边框
  的**几何**仍交给框架基类，只有背景、文字、对勾、分隔线四处自绘（深色下必须自绘：WinForms 菜单
  根本没有深色调色板，对勾更是从资源位图贴的深色字形，在深色菜单上等于看不见）
- 浅色菜单的颜色不是手挑的，而是构建调色板时从 `ProfessionalColorTable` **运行时读出来的**：
  手挑的近似值跟框架实际画的总差一截，而且这些值本来就跟着 Windows 视觉样式走
- **只改绘制，不改几何**：行高、图标列宽、弹出内边距全部交给 WinForms，菜单保持 Windows 10 的
  比例。抬高行高到 WinUI 的 32px 试过并回退了：它要跟圆角内缩行、加宽的图标列一起才成立，
  而那套几何又要 `DwmSetWindowAttribute(33)`（Windows 11 build 22000+ 专属），单独抬高只会显得空
